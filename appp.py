...
def create_goodaqi_stations_percentage(filtered_df):
    ...
    goodaqi_stations_percentage = (stations_count / total_good_occurrences)
...
fig, ax = plt.subplots(figsize=(12, 8)) 
sns.histplot(goodaqi_stations_percentage, bins=30, kde=True, color='#dcd0ff', ax=ax[0])
ax[0].set_title("Percentage of 'Good' AQI Occurrences per Station", fontsize=50)
ax[0].set_xlabel('Stations', fontsize=30)
ax[0].set_ylabel('Percentage (%)', fontsize=30)
ax[0].tick_params(axis='y', labelsize=25)
ax[0].tick_params(axis='x', labelsize=25)
fig.tight_layout(pad=10)
st.pyplot(fig)
